...
class Channel extends Core.Channel {
    ...
    async negotiate(connection) {
        let type, size, uuid;

        try {
            // First we send a READ message and get the first header
            await this.muxer.request_read(DEFAULT_UUID);
            [type, size, uuid] = await this.muxer.read_header();

            // If the first message is PROTOCOL, we'll negotiate
            if (type === MessageType.PROTOCOL) {
                await this.protocol(connection, [type, size, uuid]);
            ...
            // We're the server, accepting an incoming connection
                ...
                warning(`unexpected message type '${type}'`);
            }
    ...
    async protocol(connection, [type, size, uuid]) {
        try {
            ...
            await this.muxer.request_protocol();
        } catch (e) {
            return Promise.reject(e);
        }
    }
    ...
    async close() {
        try {
            ...
            await this.muxer.request_close(this.uuid);
        } catch (e) {
            debug(e, this.uuid);
        } finally {
...
var Transfer = class Transfer extends Channel {
    ...
    async download() {
        ...
        try {
            ...
            this.muxer.request_read(this.uuid);
    ...
    async upload(packet) {
        ...
        try {
            ...
            await this.muxer.request_open(this.uuid);
...
var Connection = class Connection {
    ...
    _recvOpen(uuid) {
        ...
        if (!channel) {
            ...
            warning('channel already open', uuid);
        }
    }
    ...
    _recvClose(uuid) {
        ...
        if (channel) {
            ...
            warning('channel already closed', uuid);
        }
    }
    ...
    _recvRead(uuid) {
        ...
        if (channel) {
            ...
            warning('no such channel', uuid);
            this.request_close(uuid);
        }
    }
        ...
        try {
            ...
            if (channel) {
                ...
                if (size > channel.read_free) {
                    warning(`Overwrite of ${size - channel.read_free}`, uuid);
                }
                ...
                this.request_read(uuid);

            // This channel doesn't exist; inform the device
            } else {
                warning('no such channel; discarding data', uuid);
                this.request_close(uuid);
            }
        } catch (e) {
            return Promise.reject(e);
